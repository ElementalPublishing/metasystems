...
Tests small, medium, and large arrays. Prints timings, speedups, and averages.
...
def run_benchmarks(N, edge0, edge1, repeat=10):
    ...
    t_numpy = benchmark(numpy_smoothstep, edge0, edge1, arr_np, repeat=repeat)
    t_cython = benchmark(smoothmath.smoothstep_array, arr_py, edge0, edge1, out_py, repeat=repeat)
    ...
    t_numpy = benchmark(numpy_smootherstep, edge0, edge1, arr_np, repeat=repeat)
    t_cython = benchmark(smoothmath.smootherstep_array, arr_py, edge0, edge1, out_py, repeat=repeat)
    ...
    t_numpy = benchmark(lambda: numpy_generalized_smoothstep(edge0, edge1, arr_np, 3), repeat=repeat)
    t_cython = benchmark(lambda: smoothmath.generalized_smoothstep_array(arr_py, edge0, edge1, 3, out_py), repeat=repeat)
...
def print_summary(all_results, sizes):
    ...
    for (size_label, N), results in zip(sizes, all_results):
        for func in ['smoothstep', 'smootherstep', 'generalized_smoothstep']:
            ...
            lines.append(f"| {size_label:<7} | {func:<23} | {avg_numpy*1e3:13.3f} | {avg_cython*1e3:18.3f} | {faster:^10} |")
    ...
    print("="*70)
    print(f"{'Size':<10} {'Function':<25} {'NumPy avg (ms)':>15} {'smoothmath avg (ms)':>20} {'Faster':>10}")
    print("-"*70)
    for (size_label, N), results in zip(sizes, all_results):
        for func in ['smoothstep', 'smootherstep', 'generalized_smoothstep']:
            t_numpy = results[func][0]
            ...
            avg_cython = sum(t_cython) / len(t_cython)
            faster = "smoothmath" if avg_cython < avg_numpy else "NumPy"
            print(f"{size_label:<10} {func:<25} {avg_numpy*1e3:15.3f} {avg_cython*1e3:20.3f} {faster:>10}")
    print("="*70)
...
def main():
    ...
    for size_label, N in sizes:
        results = run_benchmarks(N, edge0, edge1, repeat=repeat)
